...
import { ArrowLeft, Clock, Sparkles, ChevronLeft, ChevronRight } from 'lucide-react-native';
...
export default function AIQuizSessionScreen() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await apiService.quizGenerator.getQuiz(params.quizId);
        if (res.data?.success) {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await apiService.quizGenerator.submitQuizAttempt(params.quizId, {
        answers,
        timeTaken: elapsed,
      });
      if (res.data?.success) {
        ...
        router.replace({
          ...
          params: {
            ...
            score: String(result?.correctAnswers || result?.score || 0),
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      <View style={{ paddingTop: insets.top, paddingHorizontal: 16 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', paddingVertical: 12, gap: 12 }}>
          ...
          <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center', gap: 6 }}>
            ...
            <Text style={{ fontSize: 14, fontWeight: '600', color: colors.foreground }}>
              AI Quiz - Q {currentIndex + 1}/{questions.length}
            </Text>
          </View>
        ...
        {quiz?.topic && (
          <Badge variant="secondary" style={{ alignSelf: 'flex-start', marginBottom: 8 }}>{quiz.topic}</Badge>
        )}
      </View>
        ...
        <View style={{ flex: 1 }}>
          {currentIndex === questions.length - 1 ? (
            <Button onPress={() => {
              ...
              Alert.alert('Submit Quiz?', `You answered ${answered}/${questions.length} questions.`, [
